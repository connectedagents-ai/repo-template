...
|---|---|
| **MX → Microsoft 365** (likely for `netzerolending.io`, which has its own tenant) | That tenant owns the domain's mail. In order: (1) **export mailboxes** (Purview); (2) **move every reference off the domain**: change users' sign-in names (UPNs), primary and alias email addresses, shared mailboxes, groups and admin accounts to another verified domain in that tenant (Microsoft 365 refuses to remove a domain that is still in use); (3) decide where each live address should now receive mail; (4) **cut over, in one sitting**: a day ahead lower the MX record's TTL (e.g. 300 s); finish step 1.1–1.3 (Google verified, alias domain added) and step 1.8 (every live address has a user, group or routing rule) *before* touching Microsoft; then remove the domain in Microsoft 365 (M365 admin → Settings → Domains → Remove), **immediately** switch MX to Google (step 1.4), and run the step 1.9 tests. Mail sent in the minutes between removal and the MX change can bounce, so do it at a quiet hour. Skipping steps breaks sign-ins or mail silently |
